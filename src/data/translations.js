export const resources = {
  en: {
    translation: {
      //Header 
      "Select Language": "Select Language",
      "Donate": "DONATE",
      
      // Infographics
      "understanding": "UNDERSTANDING THE PROBLEM",
      "percent58": "58% of Asian adults say it's now more common for people to express racially insensitive views than before Covid.",
      "fourAdults": "More than 40% of the people stuck in the family visa backlogs are from Asia",
      "OUR IMPACT": "OUR IMPACT",
      "oneOfThree": "Helping the 97,000 Asian immigrants in the Washington D.C. and nearby areas with the intimidating naturalization process.",
      "states": "Partnering with more than 160 organizations in 33 states to foster community and advocacy",
      
      // Accordion + What we do
      "How AJ Helps": "HOW ADVANCING JUSTICE HELPS",
      "LEADERSHIP":"LEADERSHIP",
      "leadership text":"We connect communities through our leadership in coalitions at the city, state, and national levels.",
      "DIRECT SUPPORT":"DIRECT SUPPORT",
      "direct text":"We strengthen roots by increasing visibility of our community and advocating family immigration.",
      "ADVOCACY":"ADVOCACY",
      "advocacy text":"We work to defend, protect, and advance the rights fo the AAPI and other communities at all levels of government.",
      "TRAINING":"TRAINING",
      "bystander trainings text":"We equip community members with strategies to combat anti-Asian hate and harassment such as bystander intervention training.",
      "CIVIC ENGAGEMENT": "CIVIC ENGAGEMENT",
      "civic engagement text":"We grow power and mobilize for racial equity year round through our civic engagement efforts.",
      "IMMIGRATION":"IMMIGRATION RIGHTS",
      "immigration rights text":"Immigration continues to be a significant, current, life-affecting issue for AAPIs across the country.",
      "DONATE":"DONATE TODAY",
      "Help stand":"Help stand up against hate",

      // Success Stories
      "Successes":"SUCCESS STORIES",

      // Organization Links
      "Our Organizations": "OUR ORGANIZATIONS",
      "Click Below": "CLICK BELOW to visit our affiliates" ,
      "Advancing Justice": "Advancing Justice",
      "Asian Law Caucus": "Asian Law Caucus",
      "Atlanta": "Atlanta",
      "Chicago": "Chicago",
      "Los Angeles": "Los Angeles",
      "Donate Today": "DONATE TODAY",
      "AAAJ": "Asian Americans Advancing Justice",

      //Footer
      "rights": "Some rights reserved",
      "Creative Commons": "All images used are licensed under the Creative Commons License.",
      "Learn More": "Please contact your local affiliate to learn more about our work.",
    }
  },
  hi: {
    translation: {
      // Header
      "Select Language": "भाषा का चयन करें",
      "Donate": "दान करना",

      // Infographics
      "understanding": "समस्या को सुलझाने",
      "percent58": "58% एशियाई वयस्कों का कहना है कि अब लोगों के लिए कोविद की तुलना में नस्लीय रूप से असंवेदनशील विचार व्यक्त करना अधिक आम है।",
      "fourAdults": "पारिवारिक वीजा बैकलॉग में फंसे 40% से अधिक लोग एशिया से हैं",
      "OUR IMPACT": "हमारा प्रभाव",
      "oneOfThree": "वाशिंगटन डी। सी। और आस-पास के क्षेत्रों में 97,000 एशियाई प्रवासियों को डराने वाले प्राकृतिककरण प्रक्रिया में मदद करना।",
      "states": "33 राज्यों में 160 से अधिक संगठनों के साथ भागीदारी करके समुदाय और वकालत को बढ़ावा देना",

      // Accordion + What we do
      "How AJ Helps": "हम कैसे मदद करते हैं",
      "Leadership":"नेतृत्व",
      "leadership text":"हम शहर, राज्य और राष्ट्रीय स्तर पर गठबंधन में अपने नेतृत्व के माध्यम से समुदायों को जोड़ते हैं।",
      "DIRECT SUPPORT":"प्रत्यक्ष समर्थन",
      "direct text":"हम अपने समुदाय की दृश्यता और परिवार के आव्रजन की वकालत करके जड़ों को मजबूत करते हैं।",
      "ADVOCACY":"वकालत",
      "advocacy text":"हम सरकार के सभी स्तरों पर AAPI और अन्य समुदायों के अधिकारों की रक्षा, सुरक्षा और उन्नति के लिए काम करते हैं।",
      "TRAINING":"प्रशिक्षण",
      "bystander trainings text":"हम एशियाई विरोधी घृणा और उत्पीड़न का मुकाबला करने के लिए समुदाय के सदस्यों को रणनीतियों से लैस करते हैं।",
      "CIVIC ENGAGEMENT": "नागरिक अनुबंध",
      "civic engagement text":"हम अपने नागरिक जुड़ाव प्रयासों के माध्यम से नस्लीय इक्विटी वर्ष दौर के लिए शक्ति बढ़ाते हैं और जुटाते हैं।",
      "IMMIGRATION":"आव्रजन अधिकार",
      "immigration rights text":"देश भर में AAPI के लिए आव्रजन एक महत्वपूर्ण, वर्तमान, जीवन को प्रभावित करने वाला मुद्दा है।",
      "DONATE":"डोनेट टुडे!",
      "Help stand":"नफरत के खिलाफ खड़े होने में मदद करें",

      // Success Stories
      "Successes":"सफलता की कहानियां",

      // Organization Links
      "Our Organizations": "हमारे संगठन",
      "Click Below": "हमारे सहयोगियों से मिलने के लिए यहां क्लिक करें" ,
      "Advancing Justice": "न्याय को आगे बढ़ाना",
      "Asian Law Caucus": "एशियाई कानून काकस",
      "Atlanta": "अटलांटा",
      "Chicago": "शिकागो",
      "Los Angeles": "लॉस एंजिल्स",
      "Donate Today": "आज दान करें",
      "AAAJ": "एशियाई अमेरिकी न्याय को आगे बढ़ाते हुए",

      // Footer
      "Rights":"कुछ अधिकार सुरक्षित",
      "Creative Commons": "उपयोग की गई सभी छवियां क्रिएटिव कॉमन्स लाइसेंस के तहत लाइसेंस प्राप्त हैं।",
      "Learn More":"हमारे काम के बारे में अधिक जानने के लिए कृपया अपने स्थानीय सहयोगी से संपर्क करें।",
    }
  },
  vi: {
    translation: {
      //Header
      "Select Language": "Chọn ngôn ngữ",
      "Donate": "Quyên góp",

      // Infographics
      "understanding": "HIỂU VẤN ĐỀ",
      "percent58": "58% người trưởng thành châu Á nói rằng việc bày tỏ quan điểm thiếu nhạy cảm về chủng tộc ngày nay phổ biến hơn so với trước Covid.",
      "fourAdults": "Hơn 40% những người mắc kẹt trong hồ sơ visa gia đình tồn đọng đến từ Châu Á",
      "OUR IMPACT": "TÁC ĐỘNG CỦA CHÚNG TÔI",
      "oneOfThree": "Giúp 97.000 người nhập cư châu Á ở Washington D.C. và các khu vực lân cận với quá trình nhập tịch đáng sợ.",
      "states": "Hợp tác với hơn 160 tổ chức ở 33 tiểu bang để thúc đẩy cộng đồng và vận động chính sách.",
      
      // Accordion + What we do
      "How AJ Helps": "CÁCH NÂNG CAO CÔNG LÝ GIÚP",
      "LEADERSHIP":"KHẢ NĂNG LÃNH ĐẠO",
      "leadership text":"Chúng tôi kết nối các cộng đồng thông qua vai trò lãnh đạo của chúng tôi trong các liên minh ở cấp thành phố, tiểu bang và quốc gia.",
      "DIRECT SUPPORT":"HỖ TRỢ TRỰC TIẾP",
      "direct text":"Chúng tôi củng cố nguồn gốc bằng cách tăng khả năng hiển thị của cộng đồng của chúng tôi và ủng hộ việc nhập cư theo gia đình.",
      "ADVOCACY":"BIẾN HỐ",
      "advocacy text":"Chúng tôi làm việc để bảo vệ, bảo vệ và thúc đẩy các quyền của AAPI và các cộng đồng khác ở tất cả các cấp chính quyền.",
      "CIVIC ENGAGEMENT": "GIAO TIẾP CIVIC",
      "civic engagement text":"Chúng tôi phát triển quyền lực và huy động cho sự bình đẳng chủng tộc quanh năm thông qua các nỗ lực gắn kết công dân của chúng tôi.",
      "TRAINING":"ĐÀO TẠO",
      "bystander trainings text":"Chúng tôi trang bị cho các thành viên cộng đồng các chiến lược để chống lại sự thù ghét và quấy rối chống người châu Á.",
      "IMMIGRATION":"DI TRUYỀN", 
      "immigration rights text":"Nhập cư tiếp tục là một vấn đề quan trọng, hiện tại, ảnh hưởng đến cuộc sống của các AAPI trên khắp đất nước.",
      "DONATE":"TẶNG",
      "Help stand":"Giúp đứng lên chống lại sự thù ghét",

      // Success Stories
      "Successes":"CÂU CHUYỆN THÀNH CÔNG",

      // Organization Links
      "Our Organizations": "Tổ chức của chúng tôi",
      "Click Below": "NHẤP VÀO DƯỚI ĐÂY để ghé thăm các chi nhánh của chúng tôi" ,
      "Advancing Justice": "Nâng cao công lý",
      "Asian Law Caucus": "Cuộc họp kín về Luật Châu Á",
      "Atlanta": "Atlanta",
      "Chicago": "Chicago",
      "Los Angeles": "Los Angeles",
      "Donate Today": "Quyên góp ngay hôm nay",
      "AAAJ": "Người Mỹ gốc Á ủng hộ công lý",

      // Footer
      "Rights":"Một số quyền được bảo lưu",
      "Creative Commons": "Tất cả các hình ảnh được sử dụng đều được cấp phép theo Giấy phép Creative Commons.",
      "Learn More":"Vui lòng liên hệ với chi nhánh địa phương của bạn để tìm hiểu thêm về công việc của chúng tôi.",
    }
  },
  ur: {
    translation: {
      // Header
      "Select Language": "زبان منتخب کریں",
      "Donate": "عطیہ کریں",

      // Infographics
      "understanding": "مسئلے کو سمجھنا",
      "percent58": "58٪ ایشین بالغوں کا کہنا ہے کہ اب کوویڈ کے مقابلے میں نسلی طور پر غیر حساس نظریات کا اظہار کرنا لوگوں کے لئے عام ہے۔",
      "fourAdults": "خاندانی ویزا بیک لکس میں پھنسے ہوئے 40٪ سے زیادہ افراد کا تعلق ایشیاء سے ہے",
      "OUR IMPACT": "ہمارا اثر",
      "oneOfThree": "خوفناک قدرتی کاری کے عمل سے واشنگٹن ڈی سی اور قریبی علاقوں میں 97،000 ایشیائی تارکین وطن کی مدد کرنا۔",
      "states": "کمیونٹی اور وکالت کو فروغ دینے کے لئے 33 ریاستوں میں 160 سے زیادہ تنظیموں کے ساتھ شراکت کرنا",
      
      // Accordion + What we do
      "How AJ Helps": "کس طرح ایڈوانسائز جسٹس مدد",
      "LEADERSHIP":"لیڈرشپ",
      "leadership text":"ہم شہر ، ریاست اور قومی سطح پر اتحادوں میں اپنی قیادت کے ذریعہ جماعتوں کو جوڑتے ہیں۔",
      "DIRECT SUPPORT":"ڈائریکٹ سپورٹ",
      "direct text":"ہم شہر ، ریاست اور قومی سطح پر اتحادوں میں اپنی قیادت کے ذریعہ جماعتوں کو جوڑتے ہیں۔",
      "ADVOCACY":"ایڈووکیسی",
      "advocacy text":"ہم شہر ، ریاست اور قومی سطح پر اتحادوں میں اپنی قیادت کے ذریعہ جماعتوں کو جوڑتے ہیں۔",
      "TRAINING":"تربیت",
      "bystander trainings text":"ہم شہر ، ریاست اور قومی سطح پر اتحادوں میں اپنی قیادت کے ذریعہ جماعتوں کو جوڑتے ہیں۔",
      "CIVIC ENGAGEMENT": "شہری مشغولیت",
      "civic engagement text":"ہم شہر ، ریاست اور قومی سطح پر اتحادوں میں اپنی قیادت کے ذریعہ جماعتوں کو جوڑتے ہیں۔",
      "IMMIGRATION": "امیگریشن", 
      "immigration rights text":"امیگریشن ملک بھر میں AAPIs کے لئے ایک اہم ، حالیہ ، زندگی کو متاثر کرنے والا ایک اہم مسئلہ ہے۔",
      "DONATE":"آج ہی دن دیں",
      "Help stand":"نفرت کے خلاف کھڑے ہونے میں مدد کریں",

      //Success Stories
      "Successes":"کامیابی کی کہانیاں",

      // Organization Links
      "Our Organizations": "ہماری تنظیمیں",
      "Click Below": "ہمارے ملحقہ اداروں سے ملنے کے لئے نیچے کلک کریں" ,
      "Advancing Justice": "ایڈوانسنگ جسٹس",
      "Asian Law Caucus": "ایشی قانون قفقاز",
      "Atlanta": "اٹلانٹا",
      "Chicago": "شکاگو",
      "Los Angeles": "لاس اینجلس",
      "Donate Today": "آج عطیہ کریں",
      "AAAJ": "ایشیائی امریکی انصاف کی پیش کش کررہے ہیں",

      // Footer
      "rights":"کچھ حقوق محفوظ ہیں",
      "Creative Commons": "استعمال شدہ تمام تصاویر تخلیقی العام لائسنس کے تحت لائسنس یافتہ ہیں۔",
      "Learn More":"ہمارے کام کے بارے میں مزید معلومات کے ل. مقامی وابستہ سے رابطہ کریں۔",   
    }
  },
  ko: {
    translation: {
      // Header 
      "Select Language": "언어 선택",
      "Donate": "기부",

      // Infographics
      "understanding": "문제 이해",
      "percent58": "아시아 성인의 58 %는 이제 사람들이 Covid 이전보다 인종적으로 무감각 한 견해를 표현하는 것이 더 흔하다고 말합니다.",
      "fourAdults": "가족 비자 잔고에 갇힌 사람들의 40 % 이상이 아시아 출신입니다.",
      "OUR IMPACT": "우리의 영향",
      "oneOfThree": "위협적인 귀화 과정을 통해 워싱턴 D.C. 및 인근 지역에 거주하는 97,000 명의 아시아 이민자를 돕습니다.",
      "states": "33 개 주에있는 160 개 이상의 조직과 협력하여 커뮤니티 및 옹호를 촉진합니다.",

      // Accordion
      "How AJ Helps": "정의의 발전이 어떻게 도움이 되는가",
      "LEADERSHIP":"지도",
      "leadership text":"우리는 도시, 주 및 국가 차원에서 연합의 리더십을 통해 지역 사회를 연결합니다.",
      "DIRECT SUPPORT":"직접 지원",
      "direct text":"우리는 커뮤니티의 가시성을 높이고 가족 이민을 옹호함으로써 뿌리를 강화합니다.",
      "ADVOCACY":"옹호",
      "advocacy text":"우리는 모든 수준의 정부에서 AAPI 및 기타 커뮤니티의 권리를 방어, 보호 및 발전시키기 위해 노력합니다.",
      "TRAINING":"훈련",
      "bystander trainings text":"우리는 반 아시아 적 증오와 괴롭힘에 맞서 싸우기위한 전략을 지역 사회 구성원에게 제공합니다.",
      "CIVIC ENGAGEMENT": "민간 참여",
      "civic engagement text":"우리는 시민 참여 노력을 통해 연중 힘을 키우고 인종 평등을 위해 동원합니다.",
      "IMMIGRATION": "이민 권리",
      "immigration rights text":"이민은 미국 전역의 AAPI에게 중요한 현재의 삶에 영향을 미치는 문제입니다.",
      "DONATE":"오늘 기부",
      "Help stand":"증오에 맞설 수 있도록 도와주세요",

      // Success Stories
      "Successes":"성공 사례",

      // Our Organizations
      "Our Organizations": "우리 조직",
      "Click Below": "제휴사를 방문하려면 아래를 클릭하십시오" ,
      "Advancing Justice": "정의 발전",
      "Asian Law Caucus": "아시아 법률 간부",
      "Atlanta": "애틀랜타",
      "Chicago": "시카고",
      "Los Angeles": "로스 앤젤레스",
      "Donate Today": "오늘 기부",
      "AAAJ": "정의를 발전시키는 아시아계 미국인",

      // Footer
      "rights": "일부 권리 보유",
      "Creative Commons": "사용 된 모든 이미지는 Creative Commons License에 따라 사용이 허가되었습니다.",
      "Learn More": "우리의 작업에 대해 자세히 알아 보려면 현지 제휴사에 문의하십시오.",
    }
  },
};